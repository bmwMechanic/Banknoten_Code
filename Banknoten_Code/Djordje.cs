...
namespace Banknoten_Code
{
    public  class Djordje
    {
         ...
         *private string eingabe; // Private field

         // To access 'eingabe', you would need separate methods:

        public string GetEingabe()        ----------statt GetEingabeeinfach ein PUBLIC PROPERTY mit GET und SET functions ;)
        {
            return eingabe;
        }

        public void SetEingabe(string value)
        {
            eingabe = value;
        }
         */

        //FLIEDS; more is not needed
        ...
        {
            this.eingabe = Eing; //Encapsulation as well? 
        }
        ...
        {
            this.codeList[0].ToUpper();
            //codeList[0] = wandleBuchstabeInZahl(codeList[0]); //not possible!
            this.intList.Add(wandleBuchstabeInZahl());

            for (int i = 1; i < codeList.Count - 1; i++) //-1 b/c last int is check digit! works!
            {
                this.intList.Add(int.Parse(this.codeList[i]));
            }
        }
        private void CalcDigitSum()
        {
            digitsum = this.intList.Sum(); //intList ist mit dem umgewandelten LänderCode!
        }
        ...
        {
            if (int.Parse(this.codeList[11]) != checksum)    //12.Stelle des Codes! (L)(10xN)(J)
            {
                return $"Die Pruefsumme betraegt {this.checksum}\ngefaelschte Banknote!";
            }
            else
            {
                return $"Die Pruefsumme betraegt {this.checksum}\nOriginale Banknote!";
        ...
        private int wandleBuchstabeInZahl()
        {
            ...
            int zahlMit65zuViel = (int)(Convert.ToChar(this.codeList[0].ToUpper()));
            return zahlMit65zuViel - 64;
        }
        private int BerechneJ() //J is checksum
        {
            int rest = this.digitsum % 9;
            if (8 - rest == 0)
            {
                return 9;
            }
        ...
        private bool obEingabePasst()   //logic is not too beautiful but works; cannot put for
        {
            ...
            if (this.codeList.Count != 12)
            {
                return false;
            }
            else
            {
                for (int i = 1; i < this.codeList.Count; i++)
                {
                    if (!reg.IsMatch(this.codeList[i]))  //reg.IsMatch returns BOOL, so if(!BOOL) means if bool = false  :) 
                    {
